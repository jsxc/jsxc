...
export default class Location {
   ...
   public static reverseGeocodeLocation(latitude:number, longitude: number): Promise<{ street: string,
       ...
       return new Promise((resolve, reject) => {
         ...
         $.ajax({
                ...
                data: params,
				url: nominatimurl,
				error: function (request:any, textStatus:any, errorThrown:any) {
   ...
   public static distanceBetweenCoordinates(latitude1: number, longitude1: number,latitude2: number, longitude2: number): number {
      ...
      longitude2 *= pi80;
      var r = 6372.797;
      var dlat = longitude2 - latitude1;
      var dlng = latitude2 - longitude1;
      var a = Math.sin(dlat / 2) * Math.sin(dlat / 2) + Math.cos(latitude1) * Math.cos(longitude2) * Math.sin(dlng / 2) * Math.sin(dlng / 2);
