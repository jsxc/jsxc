...
export default function (contact: MultiUserContact) {
   ...
   dialog = new Dialog(`<p class="jsxc-waiting">${Translation.t('Loading_room_information')}</p>`, true,contact.getJid().bare);
   dialog.open();
   ...
   return connection
      .getMUCService()
      .getRoomConfigurationForm(contact.getJid())
...
function showForm(form: Form, contact: MultiUserContact, connection: IConnection) {
   ...
   let submitButton = $(`<div class="form-group">
      <div class="col-sm-offset-4 col-sm-8">
         <button class="jsxc-button jsxc-button--default jsxc-js-close" type="button">${Translation.t(
            'Cancel'
         )}</button>
