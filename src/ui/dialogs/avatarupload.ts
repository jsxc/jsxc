...
export default function () {
   ...
   dialog.getDom().find('select[name="account"]').on('change', (ev) => {
      ...
      dom.find('.jsxc-avatarimage').off('drop').on('drop', (ev) => {
         ev.preventDefault();
      ...
      dom.find('input[type="file"]').off('change').on('change', (ev) => {
      ...
      dom.find('form').off('submit').on('submit', ev => {
         ev.preventDefault();
      ...
      dom.find('.jsxc-js-clear').off('click').on('click', (ev) => {
         ev.preventDefault();
