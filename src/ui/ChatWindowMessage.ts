...
export default class ChatWindowMessage {
   ...
   private format(messages: IMessage[]) {
      ...
      for (let msg of messages) {
         if (msg !== undefined && (<any>msg).uid !== undefined) {
            text += msg.getPlaintextMessage() + ' - (' + DateTime.stringifyToString(msg.getStamp().getTime()) + ')\n\n';
         }
      }
      return text;
   }
   ...
   private registerHooks() {
      ...
      this.element.off('mouseup').on('mouseup', () => {
         if (new Date().getTime() >= this.longpress_start + LONGPRESS_TIME) {
            if (this.message.getDirection() === DIRECTION.OUT) {
               //we can only edit outging messages
